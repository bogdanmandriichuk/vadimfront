...
const ToListenPage = ({ isAuthenticated }) => {
    ...
    const fetchToListenAlbums = () => {
        axios.get(`http://localhost:3002/api/albums?type=to-listen&sort=${selectedSortOption}&searchArtist=${searchArtist}&searchAlbum=${searchAlbum}&searchCountry=${searchCountry}&searchYear=${searchYear}`)
            .then(response => {
    ...
    const handleMoveToListened = (albumId) => {
        if (isAuthenticated) {
            axios.put(`http://localhost:3002/api/move-to-listened/${albumId}`, null, {
                headers: {
                    Authorization: "1111" // Замініть на свій секретний ключ
                }
            })
                ...
                .catch(error => {
                    console.error('Помилка переміщення альбому:', error);
                });
        } else {
                        ...
                        <Card.Body>
                            ...
                            {isAuthenticated && (
                                <div className="d-flex justify-content-between align-items-center mt-3">
                                    ...
                                    <Button variant="primary" onClick={() => handleMoveToListened(album.id)}>Прослухав</Button>
                                </div>
                            )}
                        </Card.Body>
                    </Card>
